  ...
  return (
    <section id="community" className="scroll-scene scroll-mt-20 bg-[#f6f8fc] py-24 sm:py-32">
      <div className="mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-12">
        ...
        <div className="mt-10 grid divide-y divide-[#001d50]/15 border-y border-[#001d50]/15 sm:mt-16 md:grid-cols-3 md:divide-x md:divide-y-0">
          {communityRoles.map((role) => <article className="p-6 sm:p-9" key={role.label}><UsersThree size={28} weight="bold" className="text-[#0056c8]" /><p className="mt-8 text-xs font-black tracking-[0.16em] text-[#0056c8] sm:mt-14">{role.label}</p><h3 className="mt-3 text-3xl font-black tracking-[-0.06em]">{role.title}</h3><p className="mt-5 text-sm leading-7 text-[#23477b]">{role.description}</p></article>)}
        </div>
