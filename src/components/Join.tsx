...
export function Join() {
  ...
  return (
    <section id="join" className="scroll-scene scroll-mt-20 bg-[#8ec9ff] py-24 sm:py-32"><div className="mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-12"><p className="eyebrow">参加・連携について</p><div className="mt-6 grid gap-10 lg:grid-cols-[1.35fr_0.65fr] lg:items-end"><div><h2 className="text-[clamp(2.2rem,9.5vw,6.8rem)] font-black leading-[1.08] tracking-[-0.04em]"><span className="whitespace-nowrap">会津で、</span><br /><span className="whitespace-nowrap">一緒に動き出そう。</span></h2><p className="mt-9 max-w-xl text-lg leading-8 text-[#001d50]/75">イベントに参加したい学生も、地域で一緒に取り組みたい方も。<br />まずはReLU Matesとつながってください。</p></div><div className="grid gap-2"><PlaceholderLink href={siteConfig.socials.instagram} icon={InstagramLogo}>Instagramを見る</PlaceholderLink><PlaceholderLink href={siteConfig.socials.x} icon={PaperPlaneTilt}>Xを見る</PlaceholderLink><PlaceholderLink href={contactHref} icon={siteConfig.contactEmail ? EnvelopeSimple : ArrowUpRight}>{siteConfig.contactEmail ? '連携について相談する' : 'Instagramで相談する'}</PlaceholderLink></div></div></div></section>
  )
}
