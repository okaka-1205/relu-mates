    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[#001d50]/10 bg-white/95 text-[#001d50] backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-5 sm:h-20 sm:px-8 lg:px-12">
        <a className="flex items-center gap-3 font-black tracking-[-0.06em]" href="#top" onClick={closeMenu} aria-label="ReLU Mates トップへ">
          <img className="h-10 w-10 rounded-full bg-white object-contain sm:h-11 sm:w-11" src={siteConfig.logoImage} alt="" aria-hidden="true" />
          <span className="leading-none"><span className="block text-lg sm:text-xl">ReLU Mates</span><span className="mt-1 block text-[9px] font-bold tracking-[0.04em] text-[#0056c8] sm:text-[10px]">つながる。学ぶ。挑戦する。</span></span>
        </a>
        ...
        <button className="grid h-10 w-10 place-items-center lg:hidden sm:h-11 sm:w-11" type="button" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="mobile-navigation" aria-label={open ? 'メニューを閉じる' : 'メニューを開く'}>{open ? <X size={24} weight="bold" /> : <List size={27} weight="bold" />}</button>
      </div>
